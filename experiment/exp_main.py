...
class StandardScaler:
    ...
    def transform(self, data):
        """Scale the data using the computed mean and std."""
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(pred_loader):
                ...
                if pred_data.scale and self.args.inverse:
                    shape = outputs.shape
                    outputs = pred_data.inverse_transform(outputs.reshape(-1, shape[-1])).reshape(shape)
